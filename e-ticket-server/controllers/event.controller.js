...
const eventController = {
  ...
  getAllOrganizerEvents: async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Something went wrong to get all event of organizer" });
    }
  },
  ...
  createOrganizerEvent: async (req, res) => {
    ...
    const Event_ImagesUrls = eventImages.filter(
      (image) => image?.img_url !== undefined
    );
    ...
    if (Event_ImagesUrls.length > 0) {
      ...
      if (Event_ImagesUrls.length > 4) {
        ...
        console.log(Event_ImagesUrls);
        console.log(
          "****** there are more than 4 imges so create event blocked .."
        );
        res.status(400).json({
          error: "there are more than 4 imges so create event blocked",
        });
        return;
      }
    } else {
      ...
      res.status(400).json({
        error: "No event images defined,Sorray cannot create event !!",
      });
      return;
    }
  ...
  getEventSales: async (req, res) => {
    try {
      ...
      let sales;
      if (eventId) {
        sales = await eventService.getEventSales(
          parseInt(orgId),
          timeframe,
          parseInt(eventId)
        );
      ...
      console.error(err);
      res.status(500).json({ error: "Internal server error get events sales" });
    }
  },
  getTicketsSales: async (req, res) => {
    try {
      const { eventId } = req.params;
      const ticketsSales = await eventService.getTicketsSales(
        parseInt(eventId)
      );
