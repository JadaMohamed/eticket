...
function OrganizerSummary({ title, width, height, summaryChartStyle }) {
  ...
  const getOrganizerEvents = async () => {
    try {
      ...
      console.error(
        "OrganizerSummary: Organizer don't have any events yet",
        error
      );
    }
  };
  ...
  const getSalesData = async () => {
    try {
      const response = await Axios.get(
        `${apiUrl}/api/events/organizer/get-event-sales/${profile.user.org_id}${
          eventId !== "all" ? "?eventId=" + eventId : "?"
        }&timeframe=${getTimeFrameFromActiveBtn()}`
      );
  ...
  return (
    <div
      className="organizer-summary-container"
      //   style={{ width: width ?? 723, height: height ?? 482 }}
    >
      ...
      <div className="summary-btns">
        <div
          className={`summary-btn ${
            activeBtn === 0 ? "summary-btn-active" : ""
          }`}
        ...
        <div
          className={`summary-btn ${
            activeBtn === 1 ? "summary-btn-active" : ""
          }`}
        ...
        <div
          className={`summary-btn ${
            activeBtn === 2 ? "summary-btn-active" : ""
          }`}
